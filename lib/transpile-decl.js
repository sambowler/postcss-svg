...
module.exports = (result, promises, decl, cwd, opts) => {
	...
	const declAST = parser(decl.value).walk(
        ...
        var splicedNodes = node.nodes.splice(1);

				// <url> param()s
				const params = paramsFromNodes(splicedNodes);

        // URL exclude syntax e.g. url('/img/foo.svg' exclude)
        const nodeIsExcluded = isExcluded(splicedNodes)
        ...
        if(!nodeIsExcluded) {
          ...
          promises.push(new Promise((resolve, reject) => {
            decl.value = decl.value.replace(' exclude', '')
...
function isExcluded(nodes) {
  return nodes.reduce((sum, node) => {
    return node.type === 'word' && node.value === 'exclude';
  }, false)
}
